...
import SolanaTrading from './solanaTrading.service';
import KaminoService from './kaminoService.service';
...
class Services {
  ...
  private static injectDependencies(): void {
    ...
    // Specific dependency injection for LLM service to use Solana trading and Kamino
    if (this.instance.llmService && this.instance.solanaTrading) {
      (this.instance.llmService as any).setSolanaTrading(this.instance.solanaTrading);
    }
    if (this.instance.llmService && this.instance.kaminoService) {
      (this.instance.llmService as any).setKaminoService(this.instance.kaminoService);
    }
  }
}
